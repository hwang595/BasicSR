...
def make_hybrid_layer(basic_block, num_total_block, num_basic_block, low_rank_block, **kwarg):
    """Make layers by stacking the same blocks.
    ...
    layers = []
    #for _ in range(num_basic_block):
    for i in range(num_total_block):
        if i < num_basic_block:
            layers.append(basic_block(**kwarg))
        else:
            layers.append(low_rank_block(**kwarg))
    return nn.Sequential(*layers)
